...
### Student login
1. Go to `http://localhost:5173` → click **Student Login**.
2. Sign in with `s1@demo.hos` / `Student@123`.
3. You should land on `/student/dashboard` showing the student's name, ID, course, year and room.
4. Try visiting `http://localhost:5173/warden/dashboard` directly — you should be redirected back to `/student/dashboard` (role-based route protection).

### Warden login
1. Go to `http://localhost:5173` → click **Warden Login**.
2. Sign in with `warden@demo.hos` / `Warden@123`.
3. You should land on `/warden/dashboard` showing hostel-wide stats (total students, room occupancy, etc.) computed live from the database.
4. Visit `/warden/students` to see the searchable student list, also pulled live from the database.
5. Try visiting `http://localhost:5173/student/dashboard` directly — you should be redirected back to `/warden/dashboard`.
